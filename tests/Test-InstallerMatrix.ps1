...
if (-not (Test-Path -LiteralPath $helperPath -PathType Leaf)) {
    Write-Output 'EXPECTED_RED InstallerMatrix_HelperMissing tests/ie_fake_game.py'
    exit 1
}
if (-not (Test-Path -LiteralPath $contractPath -PathType Leaf)) {
    Write-Output 'EXPECTED_RED InstallerMatrix_ContractMissing tests/fixtures/game/publication-contract.json'
    exit 1
}

if ([string]::IsNullOrWhiteSpace($WeiduPath)) {
    throw 'Installer matrix requires an explicit WeiDU executable path.'
}
...
if ($summaryRecords.Count -ne 1) {
    throw 'Installer matrix did not emit exactly one summary record.'
}
...
if (
    ...
    throw 'Installer matrix summary did not match its PASS records.'
}
